...
| `allowedBinaries` | string[] | **Extra** binaries (e.g. `"curl"`, `"jq"`) bind-mounted read-only from `BOXY_NSJAIL_BINARIES_DIR` (`/usr/local/bin`) on the controller into the sandbox. This is **only for tools that are not already in the base rootfs** — the Ubuntu 24.04 sandbox rootfs already provides the shell and coreutils (`bash`, `sh`, `cat`, `ls`, `whoami`, …), so **do not list those here**. Each listed name must exist at `/usr/local/bin/<name>` on the controller image (pre-bake via `Dockerfile.controller.dev`); listing a name that isn't there fails sandbox creation with `copy binary "<name>": ... no such file or directory`. An empty/omitted list mounts no extras (base rootfs binaries still work). See [docs/architecture.md §4.3](docs/architecture.md). |
