...
echo ""
echo "Next step — create the two Helm mTLS secrets:"
echo ""
echo "  kubectl -n boxy create secret generic boxy-mtls-controller \\"
echo "    --from-file=ca.crt=${OUT_DIR}/ca.crt \\"
echo "    --from-file=tls.crt=${OUT_DIR}/server.crt \\"
echo "    --from-file=tls.key=${OUT_DIR}/server.key"
echo ""
echo "  kubectl -n boxy create secret generic boxy-mtls-router \\"
echo "    --from-file=ca.crt=${OUT_DIR}/ca.crt \\"
echo "    --from-file=tls.crt=${OUT_DIR}/client.crt \\"
echo "    --from-file=tls.key=${OUT_DIR}/client.key"
echo ""
echo "Then install with: helm upgrade --install boxy ./deploy/helm/boxy -n boxy --set mtls.disabled=false"
